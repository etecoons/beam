...
describe('Security Tests', () => {
  ...
  describe('Content Type Validation', () => {
    it('should handle various content types safely', async () => {
      const contentTypes = ['text/plain', 'application/json', 'image/png', 'application/pdf'];
  ...
  describe('IP Spoofing Protection', () => {
    it('should not trust X-Forwarded-For header when TRUST_PROXY is false', async () => {
      ...
      const rateLimitedOrLocked = responses.filter(
        (r) =>
          r.status === 429 || (r.status === 401 && r.data.error && r.data.error.includes('locked'))
      );

      // After 5 failed attempts, should be locked out
      assert.ok(
        rateLimitedOrLocked.length > 0,
        'Rate limiting should apply despite spoofed headers'
      );
    });
    ...
    it('should prevent rate limit bypass via header spoofing', async () => {
      ...
      const isLockedOut =
        ...
        (lastResponse.status === 401 &&
          ...
          (lastResponse.data.error.includes('locked') ||
            lastResponse.data.error.includes('Too many')));
