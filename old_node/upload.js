...
router.post('/init', async (req, res) => {
  ...
  if (isDemoMode()) {
    ...
    logger.info(
      `[DEMO] Initialized upload for ${sanitizedDemoFilename} (${fileSize} bytes) with ID ${uploadId}`
    );
  ...
  if (size > maxSizeInBytes)
    return res.status(413).json({ error: 'File too large', limit: maxSizeInBytes });

  const batchId =
    clientBatchId || `${Date.now()}-${crypto.randomBytes(4).toString('hex').substring(0, 9)}`;
  ...
  try {
    ...
    if (pathParts.length > 1) {
      ...
      if (!newFolderName) {
        ...
        try {
          ...
          if (err.code === 'EEXIST') {
            ...
            logger.info(
              `Folder "${originalFolderName}" exists or conflict, using unique "${newFolderName}" for batch ${batchId}`
            );
      ...
      if (!isPathWithinUploadDir(finalFilePath, config.uploadDir, false)) {
        logger.error(
          `Path traversal detected after folder mapping: ${pathParts.join('/')} -> ${finalFilePath}`
        );
        return res.status(403).json({ error: 'Invalid file path' });
      }
    ...
    logger.info(
      `Initialized persistent upload: ${uploadId} for ${safeFilename} -> ${finalFilePath}`
    );
    ...
    if (size === 0) {
      try {
        ...
        logger.success(
          `Completed zero-byte file upload: ${metadata.originalFilename} as ${finalFilePath}`
        );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!metadata) {
        logger.warn(
          `Upload metadata not found for chunk request: ${uploadId}. Client Batch ID: ${clientBatchId || 'none'}. Upload may be complete or cancelled.`
        );
      ...
      if (metadata.bytesReceived >= metadata.fileSize) {
        logger.warn(
          `Received chunk for already completed upload ${uploadId} (${metadata.originalFilename}). Finalizing again if needed.`
        );
        ...
        try {
          ...
          try {
            ...
            logger.info(
              `Finalized ${uploadId} on redundant chunk request (renamed ${metadata.partialFilePath} -> ${metadata.filePath}).`
            );
          } catch (renameErr) {
            if (renameErr.code === 'ENOENT') {
              logger.warn(
                `Partial file ${metadata.partialFilePath} missing during redundant chunk finalization for ${uploadId}.`
              );
            } else {
      ...
      if (metadata.bytesReceived + chunkSize > metadata.fileSize) {
        logger.warn(
          `Chunk for ${uploadId} exceeds expected file size. Received ${metadata.bytesReceived + chunkSize}, expected ${metadata.fileSize}. Truncating chunk.`
        );
        const bytesToWrite = metadata.fileSize - metadata.bytesReceived;
        chunk = chunk.slice(0, bytesToWrite);
        chunkSize = chunk.length;
        if (chunkSize <= 0) {
          // If we already have exactly the right amount
          logger.info(
            `Upload ${uploadId} already has expected bytes. Skipping write, proceeding to finalize.`
          );
      ...
      if (chunkSize > 0) {
        ...
        if (writeResult.bytesWritten !== chunkSize) {
          ...
          logger.error(
            `Partial write for chunk ${uploadId}! Expected ${chunkSize}, wrote ${writeResult.bytesWritten}. Disk full?`
          );
      ...
      logger.debug(
        `Chunk written for ${uploadId}: ${metadata.bytesReceived}/${metadata.fileSize} (${progress}%)`
      );
      ...
      if (metadata.bytesReceived >= metadata.fileSize) {
        logger.info(
          `Upload ${uploadId} (${metadata.originalFilename}) completed ${metadata.bytesReceived} bytes.`
        );
        try {
          await fs.rename(metadata.partialFilePath, metadata.filePath);
          logger.success(
          ...
          if (renameErr.code === 'ENOENT') {
            logger.warn(
              `Partial file ${metadata.partialFilePath} not found during finalization for ${uploadId}. Assuming already finalized elsewhere.`
            );
      ...
      if (fileHandle) {
        await fileHandle
          ...
          .catch((closeErr) =>
            logger.error(
              `Error closing file handle for ${uploadId} after error: ${closeErr.message}`
            )
          );
      }
    ...
    if (metadata) {
      ...
      try {
        ...
        if (unlinkErr.code !== 'ENOENT') {
          ...
          logger.error(
            `Failed to delete partial file ${metadata.partialFilePath} on cancel: ${unlinkErr.message}`
          );
        }
      }
